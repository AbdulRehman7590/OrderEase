...
//       if (typeof data.confirmationStep !== "undefined") {
//         setConfirmationStep(data.confirmationStep);
//       }
  
//       if (typeof data.orderConfirmed !== "undefined") {
...
export default function VoiceDemoPage() {
  ...
  const callBackendAPI = async (text: string): Promise<VoiceOrderResponse | null> => {
    try {
      setIsProcessing(true);
      setBackendError(null);
      ...
      const data: VoiceOrderResponse = await response.json();
      console.log("Hello bye",response);

      if (!response.ok || !data.success) {
        // throw new Error(data.error || "Backend processing failed");
        // Attempt to parse error response, if any
      let errorData: VoiceOrderResponse | null = null;
      try {
      ...
      if (typeof data.orderDetails !== "undefined") {
        setOrderDetails(data.orderDetails);
      }
